...
export class ProjectTaskService {
  ...
  async importTasks(projectId: string, files: Express.Multer.File[]) {
    const allowed = [
      'image/jpeg',
      'image/png',
      'image/webp',
    ...
    for (const file of files) {
      ...
      if (file.mimetype === 'application/pdf') {
        ...
        if (pdf.text.trim()) {
          ...
          text = await this.ocrService.recognizePdf(file.buffer);
        }
      } else {
        text = await this.ocrService.recognize(file.buffer);
      }
      fullText += '\n' + text;
    }
    ...
    const tasks = await this.taskExtractionService.extractTasks(
      fullText, //text,
    );
    ...
    const uniqueTasks = allTasks.filter(
      (task, index, array) =>
        ...
        array.findIndex(
          (x) =>
            x.title.trim().toLowerCase() === task.title.trim().toLowerCase(),
        ),
    );
    ...
    const newTasks = uniqueTasks.filter(
    ...
    const entities = newTasks.map((task) =>
      this.repo.create({
        title: task.title,
    ...
    return {
      ...
      skipped: uniqueTasks.length - entities.length,
    };
  }
  ...
  async updateTaskData(taskId: string, dto: UpdateProjectTaskDto) {
    const task = await this.repo.findOne({
      where: { id: taskId },
    });
